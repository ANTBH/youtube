...
async def init():
    ...
    LOGGER("SYSTUM").info(
        "This is a media-playing bot specifically for the "DaewaAhlalBayt" group, developed by Abdel Majeed Al Baamrani @J_dd_J."
    )
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("SYSTUM").info("Stopping media-playing DaewaAhlalBayt Bot...")
